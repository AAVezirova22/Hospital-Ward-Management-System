...
The agent reads relevant connected files, queries authorized records to resolve references, and proposes supported operations. Review every field and source, then confirm or cancel. File-derived fields can include per-field evidence with a confidence score, source location, excerpt and any conflicting citations. If a step cites a filename attached to the current request, the server marks any field without evidence as `UNRESOLVED`. The server checks citations against the current user's uploaded source context and returns the canonical filename and character offsets when an exact excerpt is present; citations it cannot verify are labeled unverified. Verified PDF citations can include a parser-derived page; verified XLSX citations can include a parser-derived sheet. Tika does not expose reliable row or column numbers in the extracted structure, so those are not presented as verified; its locations remain separately labeled `reportedLocation`. Plain text and formats without supported structural events use verified character offsets. Uncertain, conflicting, low-confidence or unverified evidence requires a field decision to accept the proposed value or edit it before confirmation. Edited values pass the same workflow validation as proposed values.

There is no autonomous confirmation tool. One confirmation applies the entire workflow transaction; errors roll back all its steps, including a new hospital. A successful new-hospital workflow opens its department. Evidence is review context only: it does not grant access, authorize a write or bypass role, ownership, expiry, capacity or current-state checks. Older proposals without field evidence retain the existing whole-workflow confirmation behavior.
...
- `GET /api/v1/assistant/provider-disclosure`: configured model mode and whether explicit patient-draft submission sends the file name and extracted text to an external provider.
- `POST /api/v1/assistant/sources`: multipart `file`; parses locally and returns source ID, name, character count and expiry. Upload alone does not call the model.
- `DELETE /api/v1/assistant/sources/{id}`: remove a source owned by the current user and department.
- `POST /api/v1/assistant/patient-drafts`: `{ "sourceId": "..." }`; explicitly submits an owned source for extraction and returns an unsaved review draft with per-field status, confidence, source excerpts, match candidates and source-supported follow-up actions.
- `GET /api/v1/assistant/patient-drafts/{draftId}`: reopen the short-lived draft for its owner in the same department; includes `patientId` when bound.
- `PUT /api/v1/assistant/patient-drafts/{draftId}/patient`: `{ "patientId": 123 }`; binds the transient draft to the reviewed patient identity.
...
- Each workflow step may include `evidence` keyed by field. Entries have `status` (`SUPPORTED`, `UNCERTAIN`, `CONFLICT`, `UNRESOLVED`), `confidence`, `sources`, `conflicts` and server-computed `requiresDecision`. A citation reports `sourceId`, `sourceName`, `location`, `reportedLocation`, `excerpt`, `verified`, and character offsets when verified. `verified` means the exact excerpt was found in a source attached to the current request; `location` is a parser-derived PDF page or XLSX sheet when available, otherwise a verified character range. Tika does not provide reliable row/column provenance; model-provided positions appear separately in `reportedLocation` and are not verified.
- Existing `/api/v1/ai-actions/{id}/confirm` and `/cancel` apply ownership, role, expiry and replay checks. Confirmation revalidates the workflow and executes through business services. For fields with `requiresDecision: true`, POST `/api/v1/ai-actions/{id}/confirm` accepts `{ "fieldDecisions": [{"stepKey":"p1","field":"firstName","decision":"ACCEPTED"}] }` or `decision: "EDITED"` with a `value`. Every required field needs exactly one decision.
