...
- Canvas UI's actual [Clouds React/WebGL source](https://canvasui.dev/docs/components/clouds), vendored from its official registry, creates the atmospheric layer over architectural images. The component and shared rectangle-cache helper retain David Haz's [MIT + Commons Clause license](../frontend/src/vendor/canvasui/LICENSE.md).
- Lucide is the locked icon set for operational chrome (sidebar, tables, assistant). Do not replace it with custom SVG without a design review; the previous skill note against icon libraries does not apply to this repo.
