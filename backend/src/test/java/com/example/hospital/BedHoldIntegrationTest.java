...
var dashboard =
    result(request("admin", "GET", "/api/v1/reports/dashboard", null), 200);
...
void availabilityFiltersAndPaginationUsePeakReservationsWithCapabilities() throws Exception {
        ...
        result(
            request(
                ...
                Map.of(
                    ...
                    "bedCount", 4,
                    "active", true,
                    "capabilities", java.util.List.of("oxygen"))),
            201);
  ...
  result(
      request(
          "admin",
          "POST",
          "/api/v1/rooms/" + ids.get(0) + "/holds",
          Map.of(
              "bedCount", 4,
              "reason", "Full",
              "startsAt", now.minusSeconds(60).toString(),
  ...
  for (int i = 1; i <= 2; i++) {
    result(
        request(
            "admin",
            "POST",
            "/api/v1/rooms/" + ids.get(1) + "/holds",
            Map.of(
                "bedCount", 2,
                "reason", "Adjacent " + i,
                "startsAt", now.plusSeconds(i * 3600).toString(),
                "endsAt", now.plusSeconds((i + 1) * 3600).toString())),
        201);
  }
...
var minFree =
    result(
        request(
            "admin",
            "GET",
            "/api/v1/rooms?minFree=2&roomId=" + room.get("id").asLong(),
            null),
        200);
assertThat(
        java.util.stream.StreamSupport.stream(minFree.get("items").spliterator(), false)
...
void adjacentMaintenanceWindowsDoNotDoubleCountTheSameCapacity() throws Exception {
  ...
  var held =
      roomCapacity(room.get("id").asLong()).get("heldBeds").asInt();
  ...
  var filtered =
      result(
          request(
              "admin",
              "GET",
              "/api/v1/rooms?minFree=1&roomId=" + room.get("id").asLong(),
              null),
          200);
