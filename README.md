...
For Google AI Studio, use `AI_MODE=external`, `AI_URL=https://generativelanguage.googleapis.com/v1beta/openai/chat/completions`, and `AI_MODEL=gemini-3.5-flash`. Put your key in `AI_API_KEY` in the ignored root `.env` and recreate the backend. The key is passed only to the Java backend. A Google response of HTTP 403 with `PERMISSION_DENIED` requires resolving access for the Google project; restarting Medcore does not grant provider access. See [Google's compatibility documentation](https://ai.google.dev/gemini-api/docs/openai).
